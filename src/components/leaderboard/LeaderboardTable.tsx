      ...
      <table className="w-full">
        <thead>
          <tr className="bg-bwb-surface-2 text-left">
            ...
            <th className="px-4 py-3 text-xs font-display uppercase tracking-wider text-bwb-muted text-center">Status / Prize</th>
            <th className="px-4 py-3 text-xs font-display uppercase tracking-wider text-bwb-muted text-right">Score</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((team, i) => {
              ...
              <React.Fragment key={team.id}>
                ...
                >
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      {showMovement && team.rankChange !== undefined && team.rankChange !== 0 && (
                        <span className={`text-[10px] font-mono font-bold ${team.rankChange > 0 ? 'text-bwb-success' : 'text-bwb-danger'}`}>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2">
                      <span className="font-display font-bold text-sm text-bwb-text">{team.name}</span>
                      {team.department && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-white/5 text-bwb-muted hidden sm:inline">
                          {team.department}
                        </span>
                      )}
                    </div>
                  ...
                  {/* Members Column */}
                  {!compact && (
                    <td className="px-4 py-3.5 text-xs text-bwb-muted hidden sm:table-cell">
                  ...
                  <td className="px-4 py-3.5 text-center">
                    {round === 3 || isFinalResults ? (
                      isFirst ? (
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-black bg-bwb-gold text-bwb-bg shadow-sm">
                          🏆 1ST CHAMPION
                        </span>
                      ) : isSecond ? (
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold bg-slate-400/20 text-slate-200 border border-slate-400/30">
                          🥈 2ND RUNNER-UP
                        </span>
                      ) : isThirdOrFourth ? (
                        ...
                        <span className="px-2 py-0.5 rounded-lg text-[10px] font-mono text-bwb-muted bg-white/5 border border-white/10">
                          🎖️ Finalist #{rank}
                        </span>
                      )
                    ) : round === 2 ? (
                      isTop8 ? (
                        ...
                        <span className="px-2 py-0.5 rounded-lg text-[10px] font-mono text-bwb-muted bg-white/5">
                          Rank #{rank}
                        </span>
                      )
                    ) : (
                      <span className="px-2 py-0.5 rounded-lg text-[10px] font-mono font-semibold bg-bwb-accent/15 text-bwb-accent border border-bwb-accent/20">
                        Advances to R2
                      </span>
                    )}
                  </td>
                  ...
                  {/* Score Column */}
                  <td className="px-4 py-3.5 text-right font-display font-bold text-sm sm:text-base text-bwb-accent">
                    {team.score ?? 0}
                    <span className="text-[10px] text-bwb-muted font-mono font-normal"> pts</span>
                  </td>
                </motion.tr>
                ...
                {showRound2Cutoff && (
                  <tr className="bg-emerald-500/10 border-y-2 border-emerald-500/40">
                    <td colSpan={compact ? 4 : 5} className="py-2 px-4 text-center">
                      <span className="text-xs font-mono font-black text-emerald-400 uppercase tracking-wider flex items-center justify-center gap-2">
