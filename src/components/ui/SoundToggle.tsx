...
export function SoundToggle() {
  ...
  return (
    <button
      ...
      className={`fixed bottom-4 right-4 z-50 p-2.5 rounded-2xl border backdrop-blur-xl transition-all shadow-xl flex items-center gap-2 text-xs font-mono font-bold ${
        muted
          ? 'bg-bwb-surface-2/90 text-bwb-muted border-white/10 hover:text-bwb-text hover:border-bwb-accent/40'
          : 'bg-purple-600/90 text-white border-purple-400 shadow-purple-500/30 hover:scale-105'
      }`}
    >
      {muted ? (
        <>
